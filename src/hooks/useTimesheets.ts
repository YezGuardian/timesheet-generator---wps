...
` +
    ...
    const handleUploadTimesheet = async (
        ...
        try {
            ...
            const storagePath = `companies/${encodeURIComponent(candidate.company)}/${candidate.id}/${fileName}`;
            const storageRef = ref(storage, storagePath);

            // --- NEW: Delete old file if it exists ---
            if (timesheet.uploaded && timesheet.uploadedFileName) {
                const oldFileName = timesheet.uploadedFileName;
                const oldStoragePath = `companies/${encodeURIComponent(candidate.company)}/${candidate.id}/${oldFileName}`;
                const oldStorageRef = ref(storage, oldStoragePath);
                try {
                    await deleteObject(oldStorageRef);
                    console.log(`Old file ${oldFileName} deleted successfully.`);
                } catch (deleteError: any) {
                    // Handle case where old file might not exist (e.g., first upload after migration)
                    if (deleteError.code === 'storage/object-not-found') {
                        console.warn(`Old file ${oldFileName} not found, skipping deletion.`);
                    } else {
                        console.error(`Error deleting old file ${oldFileName}:`, deleteError);
                        // Decide if you want to throw or continue. For now, continue.
                    }
                }
            }
            // --- END NEW ---
            ...
            // --- NEW: Prepare upload history ---
            const currentUploadHistory = timesheet.uploadHistory || [];
            const newUploadLogEntry = {
                fileName: fileName,
                uploadedAt: newUploadedAt,
            };
            const updatedUploadHistory = [...currentUploadHistory, newUploadLogEntry];
            // --- END NEW ---
            ...
            const updatedTimesheets = candidate.timesheets.map(t => 
                t.id === timesheet.id 
                    ? { 
                        ...
                        uploadedAt: newUploadedAt,
                        uploadHistory: updatedUploadHistory,
                    }
                    : t
            );
            await updateDoc(candidateRef, { timesheets: updatedTimesheets });
            
            alert(`✅ Timesheet uploaded/updated successfully!
